...
class EmpleadoModel {
    ...
    static async getAll() {
        const result = await pool.query(`
            SELECT
                ...
                e.fi_departamento_id, e.fi_puesto_id,
    ...
    static async create(data) {
        const {
            fi_usuario_id, fi_departamento_id, fi_puesto_id,
    ...
    static async update(id, data) {
        const {
            ...
            fd_fecha_contratacion, fn_uniformes
        } = data;
        ...
        const result = await pool.query(`
            UPDATE rrhh.empleados SET
                ...
                fi_puesto_id = $2,
                fc_nombre = $3,
                fc_apellido_paterno = $4,
                fc_apellido_materno = $5,
                fc_genero = $6,
                fd_fecha_nacimiento = $7,
                fc_estado = $8,
                fc_ciudad = $9,
                fc_calle = $10,
                fc_codigo_postal = $11,
                fc_referencias = $12,
                ft_comentarios_adicionales = $13,
                fd_fecha_contratacion = $14,
                fn_uniformes = $15
            WHERE fi_empleado_id = $16
            RETURNING *;
        `, [
            fi_departamento_id, fi_puesto_id || null,
            fc_nombre, fc_apellido_paterno, fc_apellido_materno,
            ...
            fd_fecha_contratacion || null, fn_uniformes ?? 0,
            id
        ]);
        return result.rows[0];
    }
    ...
    static async deactivate(id) {
        const result = await pool.query(`
            UPDATE rrhh.empleados SET fb_activo = false
            WHERE fi_empleado_id = $1 RETURNING *;
        `, [id]);
        return result.rows[0];
    }
    ...
    static async activate(id) {
        const result = await pool.query(`
            UPDATE rrhh.empleados SET fb_activo = true
            WHERE fi_empleado_id = $1 RETURNING *;
        `, [id]);
        return result.rows[0];
    }
