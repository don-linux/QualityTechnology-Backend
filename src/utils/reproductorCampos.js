...
export function parseReproductorCampos(body) {
  ...
  return {
    ...
    fecha_siembra: toDateOrNull(
      pick(body, "fecha_siembra", "fecha_siembra_reproductores"),
    ),
    lote_genetico: sliceStr(pick(body, "lote_genetico"), 120),
    activo,
    ...(desovez !== undefined ? { desovez } : {}),
    ...(estado_ciclo !== undefined ? { estado_ciclo } : {}),
    genetica_machos: sliceStr(pick(body, "genetica_machos"), 60),
    familia_machos: sliceStr(pick(body, "familia_machos"), 60),
    procedencia_machos: sliceStr(pick(body, "procedencia_machos"), 100),
    genetica_hembras: sliceStr(pick(body, "genetica_hembras"), 60),
    familia_hembras: sliceStr(pick(body, "familia_hembras"), 60),
    procedencia_hembras: sliceStr(pick(body, "procedencia_hembras"), 100),
    ratio: calcularRatioReproductor(machos, hembras),
    talla: toDecimalRepro(pick(body, "talla")),
  };
}
